...
export default function AddCommentForm({articleName, setArticleInfo}) {
  ...
  const addComments = async () => {
    const result = await fetch(`api/articles/${articleName}/addComments`, {
      method: 'post',
  ...
  return (
    <form action="" className="shadow rounded px-8 pt-6 pb-8 mb-4">
      ...
      <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} name="" id="" rows="4" cols="50" className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" /> 
